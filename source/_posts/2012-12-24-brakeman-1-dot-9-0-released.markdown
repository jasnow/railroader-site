---
layout: post
title: "Railroader 1.9.0 Released"
 ...
 * Support `strong_parameters` ([#88](https://github.com/presidentbeef/railroader/issues/88))
 * Support newer `validates :format` call ([#198](https://github.com/presidentbeef/railroader/issues/198))
 * Add scan time to reports ([#158](https://github.com/presidentbeef/railroader/issues/158))
 * Add Railroader version to reports
 * Don't mangle whitespace in JSON code formatting
 * Fix `CheckExecute` to warn on all string interpolation ([#213](https://github.com/presidentbeef/railroader/issues/213))
...
With the update to use RubyParser 3.x, Railroader no longer includes a vendored version of RubyParser (which was only used with running with Ruby 1.9), which reduced code size by about 14,000 lines.

RubyParser 3 supports Ruby 1.9 much more fully, so there should be very few parse errors. Additionally, Railroader no longer chooses parsers based on the current Ruby version.

([changes](https://github.com/presidentbeef/railroader/pull/190))

### Route Information Ignored by Default

Route information is only used in Railroader to determine whether a controller method should perform an implicit render. Since determining Rails routes statically is quite difficult to get right, it is better to assume a method is a routable action. Previous versions provided the `-a` option for this, which is now on by default. To turn off this behavior (and revert to the old), use `--no-assume-routes`.

This does not affect default route warnings.

([changes](https://github.com/presidentbeef/railroader/pull/219))

### Optional Interprocedural Analysis for Controllers

Currently, Railroader essentially looks at each method in isolation. Values are propagated from controller actions (including before\_filters) through rendered views and templates. But if a controller action calls another method which has an effect on the eventual output of the action, Railroader does not processed the invoked method.
...
([changes](https://github.com/presidentbeef/railroader/pull/218))
...
([changes](https://github.com/presidentbeef/railroader/pull/224))

### Support for `strong_parameters`

In Rails 4, the responsibility for mass assignment protection will move from models to controllers where mass assignment is actually used. A preview of the new functionality is available in the [strong_parameters](https://github.com/rails/strong_parameters) gem. Railroader should no longer warn on mass assignment for models protected with `strong_parameters`. 

([changes](https://github.com/presidentbeef/railroader/pull/204))

### Support `validates :format`

In Rails 3, format validation changed from `validates_format_of` to the more generic `validates` method. Railroader will now check `validates` calls for proper anchors on regular expressions for `:format`.

([changes](https://github.com/presidentbeef/railroader/pull/205))

### Report Changes

Reports will now contain Railroader version and scan duration. JSON reports contain `start_time` and `end_time` timestamps. For now, JSON reports will still have a `timestamp` key, but it will be identical to `end_time`. It will be removed in Railroader 2.0.

([changes](https://github.com/presidentbeef/railroader/pull/193))
...
([changes](https://github.com/presidentbeef/railroader/pull/191))
...
([changes](https://github.com/presidentbeef/railroader/pull/216))

### Fix `to_sql` False Positives

SQL code generated from Arel's `to_sql` method will be considered safe.

([changes](https://github.com/presidentbeef/railroader/pull/194))

### File System Facade

Bryan Helmkamp cleaned up Railroader's file access into a single object. This should simplify future changes and add some consistency to how Railroader handles files.

([changes](https://github.com/presidentbeef/railroader/pull/197))

### Run Tests via Rake

Thanks to Michael Grosser, running `rake` will now run Railroader's tests. 

([changes](https://github.com/presidentbeef/railroader/pull/183))

### Improved Config File Locations

Also thanks to Michael Grosser, Railroader will check more sane locations for Railroader configuration files. This version adds `./config/railroader.yml`, `~/.railroader/config.yml`, and `/etc/railroader/config.yml` as default locations for configuration files. The old locations are deprecated now, and will be removed in Railroader 2.0.

([changes](https://github.com/presidentbeef/railroader/pull/182)
...
([changes](https://github.com/presidentbeef/railroader/pull/189))

The number of s-expressions generated by Railroader has been reduced, although this did not lead to any major performance improvement.

([changes](https://github.com/presidentbeef/railroader/pull/207))
